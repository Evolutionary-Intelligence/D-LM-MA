...
class LMMAES(ES):
    ...
    def __init__(self, problem, options):
        ...
        self.c_s = options.get('c_s')  # c_sigma in Algorithm 1
        ...
        self.s = np.copy(options.get('s'))  # evolution path (p in Algorithm 1)
        self.tm = np.copy(options.get('tm'))  # transformation matrix M

    def _set_c_s(self):
        c_s = 2 * self.n_individuals / self.ndim_problem
        if c_s < 0:
            c_s = 0
        elif c_s > 1:
            c_s = 1
    ...
    def _set__s_2(self):
        s_2 = self._mu_eff * self.c_s * (2 - self.c_s)
        if s_2 < 0:
            s_2 = 0
        return np.sqrt(s_2)
    ...
    def _update_distribution(self, z=None, d=None, mean=None, s=None, tm=None, y=None):
        ...
        for k in range(self.n_evolution_paths):  # rank-m
            c_c_2 = self._mu_eff * self._c_c[k] * (2 - self._c_c[k])
            if c_c_2 < 0:
                c_c_2 = 0
            tm[k] = (1 - self._c_c[k]) * tm[k] + np.sqrt(c_c_2) * z_w
